...
//                client.stopContainerCmd(container.getId()).exec();
    ...
    public List<CiWork> getCIList(int repoId) {
        return jdbcTemplate.query("select id as id,ci_name as ci_name,output_hash as output_hash, created_at as created_at from ci_log where repo_id = ? order by id desc", CiWork.mapper, repoId);
    }
